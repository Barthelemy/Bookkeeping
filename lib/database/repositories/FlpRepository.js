...
class FlpRepository extends Repository {
     ...
     *
     * @param {Object} runNumber The QueryBuilder to use.
     * @param {QueryBuilder} queryBuilder The QueryBuilder to use.
     * @returns {Promise} Promise object representing the full mock data
     */
    async findAllByRunNumber(runNumber, queryBuilder = new QueryBuilder()) {
        queryBuilder
            .setModel(FlpRoles)
            .include('runs')
            .whereAssociation('runs', 'runNumber').is(runNumber);
