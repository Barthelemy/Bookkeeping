...
class RunTagsRepository extends Repository {
    ...
    async findAllAndGroup(queryBuilder = new QueryBuilder()) {
        ...
        const groupedResult = result.reduce((accumulator, currentValue) => {
            if (accumulator[currentValue.runNumber]) {
                accumulator[currentValue.runNumber].push(currentValue.tagId);
            } else {
                accumulator[currentValue.runNumber] = [currentValue.tagId];
            }
            return accumulator;
        }, {});

        return Object.entries(groupedResult).map(([key, value]) => ({ runNumber: key, tagIds: value }));
    }

    /**
     * Remove all tags & runs relations by run id
     * @param {number} runNumber  - run number to delete tags for
     * @return {Promise<void>} - delete result
     */
    async removeByRunNumber(runNumber) {
        return await RunTags.sequelize.getQueryInterface().bulkDelete('run_tags', { run_number: runNumber });
    }
