...
class CreateLogUseCase {
    ...
    async execute(dto) {
        ...
        const log = await TransactionHelper.provide(async () => {
            ...
            if (runNumbers) {
                ...
                for (const runNumber of numbers) {
                    ...
                    runNumbersList.push(run.id);
                }

                body.runs = runNumbersList;
            }
        ...
        if (!log.error && body.runs && body.runs.length > 0) {
            await TransactionHelper
                .provide(() => LogRunNumberRepository
                    .bulkInsert(body.runs.map((runNumber) => ({ runNumber, logId: log.id }))));
        }
