...
const { RunAdapter } = require('../../domain').adapters;
const { QueryBuilder, TransactionHelper } = require('../../database').utilities;
...
class GetRunByRunNumberUseCase {
    ...
    async execute(dto) {
        ...
        const queryBuilder = new QueryBuilder().where('run_number').is(runNumber);
