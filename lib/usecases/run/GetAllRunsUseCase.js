...
class GetAllRunsUseCase {
    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            const { runNumber, environmentId, o2start, o2end, nDetectors, nFlps, ddflp, dcs, epn, epnTopology, detectors } = filter;
        ...
        const {
            ...
            if (filter && filter.tag) {
                ...
                tagQueryBuilder.where('tagId').oneOf(...filter.tag.values).orderBy('runNumber', 'asc');

                let runTags;
                switch (filter.tag.operation) {
                    case 'and':
                        runTags = await RunTagsRepository.findAllAndGroup(tagQueryBuilder);
                        runTags = runTags.filter((runTags) => filter.tag.values.every((tagId) => runTags.tagIds.includes(tagId)));
                        break;
                    case 'or':
                        runTags = await RunTagsRepository.findAll(tagQueryBuilder);
                        break;
                }

                const runNumbers = runTags.map((runTags) => runTags.runNumber);
