...
const collapsedButton = (model, post) => h('a.btn.btn-primary', {
    ...
    onclick: () => {
        model.logs.collapsePostById(post.id);
    },
}, 'Collapse');
...
const textBox = (index, model, post) => h(`div#post-id${index}`, h(`textarea.w-100#post-content${index}`, {
    oninit: () => {
        ...
        model.log.clearSingleEditor();
        model.log.setMarkdownBox(`post-content${index}`, eventHandler, readOnlyProps, true);
    },
    onremove: () => {
        ...
        model.log.removeMarkdownBox(`post-content${index}`);
    },
}, post.text));
