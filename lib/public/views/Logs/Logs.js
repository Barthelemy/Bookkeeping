...
import ParamsBuilder from '../../utilities/ParamsBuilder';
...
export default class Overview extends Observable {
    ...
    async fetchFilteredLogs() {
        ...
        const tagFilterIsActive = this._tagFilterValues.length > 0;
        const runFilterIsActive = this._runFilterValues.length > 0;
        builder.addParameter('filter[title]', this._titleFilterText, this._titleFilterText);
        builder.addParameter('filter[author]', this._authorFilterText, this._authorFilterText);
        builder.addParameter('filter[created][from]', new Date(`${this._createdFilterFrom.replace(/\//g, '-')}T00:00:00.000`).getTime(), this._createdFilterFrom);
        builder.addParameter('filter[created][to]', new Date(`${this._createdFilterTo.replace(/\//g, '-')}T23:59:59.999`).getTime(), this._createdFilterTo);
        builder.addParameter('filter[tag][values]', this._tagFilterValues.join(), tagFilterIsActive);
        builder.addParameter('filter[tag][operation]', this._tagFilterOperation.toLowerCase(), tagFilterIsActive);
        builder.addParameter('filter[run][values]', this._runFilterValues.join(), runFilterIsActive);
        builder.addParameter('filter[run][operation]', this._runFilterOperation.toLowerCase(), runFilterIsActive);
        builder.addParameter('sort[${this.sortingColumn}]', this._sortingOperation, this._sortingColumn && this._sortingOperation);
        builder.addParameter('page[offset]', this._logs.payload && this._logsPerPage === Infinity ?
            this._logs.payload.length : (this._selectedPage - 1) * this._logsPerPage, true);
        builder.addParameter('page[limit]', this._logsPerPage === Infinity ? this.model.INFINITE_SCROLL_CHUNK_SIZE : this._logsPerPage, true);

        const result = await this.logsService.fetchAllLogs(builder.build());
        if (result.data) {
