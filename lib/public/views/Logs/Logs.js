...
export default class Overview extends Observable {
    ...
    async fetchFilteredLogs() {
        ...
        builder.addStartDate('filter[created][from]', this._createdFilterFrom);
        builder.addEndDate('filter[created][to]', this._createdFilterTo);
        if (this._tagFilterValues.length > 0) {
            builder.addParameter('filter[tag][values]', this._tagFilterValues.join());
            builder.addParameter('filter[tag][operation]', this._tagFilterOperation.toLowerCase());
        }
        if (this._runFilterValues.length > 0) {
            builder.addParameter('filter[run][values]', this._runFilterValues.join());
            builder.addParameter('filter[run][operation]', this._runFilterOperation.toLowerCase());
        }
