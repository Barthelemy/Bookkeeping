...
const logOverviewScreen = (model) => {
    ...
    const { amountDropdownVisible } = model.logs.amountDropdownVisible;
    const { logsPerPage } = model.logs;
    ...
    return h('', { onremove: () => model.logs.clearLogs() }, [
        ...
        h('.flex-row.header-container', [
            ...
            h('button.btn.btn-primary.mt2.ml2', {
                ...
                id: 'openFilterToggle',
            }, `${model.logs.showFilters ? 'Close' : 'Open'} filters`),
            h(`.w-25.filters
            ${model.logs.showFilters ? '.display-block' : '.display-none'}`, {
                onclick: () => {
                    // eslint-disable-next-line no-undef
                    window.clickedInFilters = true;
                    setTimeout(() => {
            ...
            && !model.logs.showFilters ? '.display-block' : '.display-none'}`, h('.f5'), `Active filters:
            ${model.logs.activeFilters.join(', ')}`),
