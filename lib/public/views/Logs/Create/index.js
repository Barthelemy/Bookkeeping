...
const mdBox = (model, text) => h('', [
    h('textarea#text.form-control', {
        ...
        disabled: true,
        onchange: (e) => {
            model.log.setText(e.target.value);
        },
        oninit: () => {
            model.log.setMarkdownBox('text', { location: 'log', name: 'setText' }, { isReadOnly: false });
        },
    }, text),
]);
...
const createLogScreen = (model) => {
    const { title, text, selectedTags, attachments } = model.log;
    const allTags = model.tags.getTags();
    const runNumber = model.router.params.id;
    let { runNumbers } = model.log;
    ...
    return h('div#create-log', [
        ...
        h('', {
            onremove: () => model.log.clearAllEditors(),
        }, [
            h('h2', 'Create Log'),
            ...
            h('input#title.form-control.w-100', {
                ...
                value: title,
                oninput: (e) => {
                    model.log.title = e.target.value;
                },
            }, title),
            ...
            h('input#run-number.form-control.w-100', {
                ...
                value: runNumbers,
                oninput: (e) => {
                    model.log.runNumbers = e.target.value;
                },
            ...
            h('label.form-check-label.f6', { for: 'tags' }, [
                'Add one or multiple tags to the log. ',
            ...
            h('select#tags.form-control', {
                multiple: true,
                onchange: (e) => {
                    model.log.selectedTags = e.target.selectedOptions;
                },
            ...
            h('.flex-row.justify-between', [
                h('input#attachments.form-control', {
                    ...
                    multiple: true,
                    onchange: (e) => {
                        model.log.attachments = e.target.files;
                    },
                }),
                attachments.length > 0 && h('button#clearAttachments.btn.btn-danger.ml3', {
                    onclick: () => model.log.clearAttachments(),
                }, 'Clear'),
            ]),
            h('#attachmentNames', {
