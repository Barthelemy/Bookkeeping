...
const runDetails = (model) => {
    ...
    const data = model.runs.run;
    const dataLogs = model.run.logsOfRun;
    const dataFlps = model.run.flpsOfRun;
    ...
    const selectedTags = model.run.getSelectedTags();
                ...
                h('.flex.w-50', [
                    ...
                    h('.flex-row.mv2', [
                        h('button.btn.btn-success.w-30.mh2', {
                            onclick: () => model.run.updateRunTags(),
                            title: 'This action will overwrite existing tags',
                            disabled: !model.run.selectedTagsChanged(),
                        }, 'Update Tags'),
                    ...
                    h('select#tags-control.form-control.w-50.mh2', {
                        multiple: true,
                        onchange: ({ target }) => model.run.setSelectedTags(target.selectedOptions),
