...
export default class Overview extends Observable {
    ...
    async fetchAllRuns() {
        ...
        builder.addParameter('filter[runNumber]', this.runFilterValues);

        if(this.tagFilterValues.length > 0) {
            builder.addParameter('filter[tag][values]', this.tagFilterValues.join());
            builder.addParameter('filter[tag][operation]', this.tagFilterOperation.toLowerCase());
        }

        builder.addStartDate('filter[o2start][from]', this.o2startFilterFrom);
        builder.addEndDate('filter[o2start][to]', this.o2startFilterTo);
        builder.addStartDate('filter[o2end][from]', this.o2endFilterFrom);
        builder.addEndDate('filter[o2end][to]', this.o2endFilterFrom);
        builder.addParameter('filter[nDetectors]', this.nDetectorsFilterValues);
        builder.addParameter('filter[environmentId]', this.enviromentIdFilter);
        builder.addParameter('filter[nFlps]', this.nFlpsFilterValues);
        builder.addParameter('filter[ddflp]', this.ddflpFilter);
        builder.addParameter('filter[dcs]', this.dcsFilter);
        builder.addParameter('filter[epn]', this.epnFilter);
        builder.addParameter('filter[epnTopology]', this.epnTopologyFilter);
        builder.addParameter('filter[detectors]', this.detectorsFilterValues);
        ...
        builder.addParameter('page[limit]', this.isInfiniteScrollEnabled() ? this.model.INFINITE_SCROLL_CHUNK_SIZE : this.runsPerPage);
    ...
    async getFilteredRuns(runs) {
        ...
        return selectedRuns.map((selectedRun) => pick(selectedRun, this.getSelectedRunsFields()));
    }
    ...
    setSelectedRunsFields(selectedOptions) {
        ...
        for (const value of selectedOptions) {
            this.selectedRunsFields.push(value);
        }
