...
import ParamsBuilder from '../../utilities/ParamsBuilder.js';
...
export default class Overview extends Observable {
    ...
    async fetchAllRuns() {
        ...
        const tagFilterActive = this.tagFilterValues.length > 0;
        builder.addParameter('filter[runNumber]', this.runFilterValues, this.runFilterValues);
        builder.addParameter('filter[tag][values]', this.tagFilterValues.join(), tagFilterActive);
        builder.addParameter('filter[tag][operation]', this.tagFilterOperation.toLowerCase(), tagFilterActive);
        builder.addParameter('filter[o2start][from]', new Date(`${this.o2startFilterFrom.replace(/\//g, '-')}T00:00:00.000`).getTime(), this.o2startFilterFrom);
        builder.addParameter('filter[o2start][to]', new Date(`${this.o2startFilterTo.replace(/\//g, '-')}T23:59:59.999`).getTime(), this.o2startFilterTo);
        builder.addParameter('filter[o2end][from]', new Date(`${this.o2endFilterFrom.replace(/\//g, '-')}T00:00:00.000`).getTime(), this.o2endFilterFrom);
        builder.addParameter('filter[o2end][to]', new Date(`${this.o2endFilterTo.replace(/\//g, '-')}T23:59:59.999`).getTime(), this.o2endFilterFrom);
        builder.addParameter('filter[nDetectors]', this.nDetectorsFilterValues, this.nDetectorsFilterValues);
        builder.addParameter('filter[environmentId]', this.enviromentIdFilter, this.enviromentIdFilter);
        builder.addParameter('filter[nFlps]', this.nFlpsFilterValues, this.nFlpsFilterValues);
        builder.addParameter('filter[ddflp]', this.ddflpFilter, this.ddflpFilter);
        builder.addParameter('filter[dcs]', this.dcsFilter, this.dcsFilter);
        builder.addParameter('filter[epn]', this.epnFilter, this.epnFilter);
        builder.addParameter('filter[epnTopology]', this.epnTopologyFilter, this.epnTopologyFilter);
        builder.addParameter('filter[detectors]', this.detectorsFilterValues, this.detectorsFilterValues);
        builder.addParameter('page[offset]', this.runs.payload && this.isInfiniteScrollEnabled() ?
            this.runs.payload.length : (this.selectedPage - 1) * this.runsPerPage, true);
        builder.addParameter('page[limit]', this.isInfiniteScrollEnabled() ? this.model.INFINITE_SCROLL_CHUNK_SIZE : this.runsPerPage, true);
        const params = builder.build();

        const result = await this.runsService.fetchAllRuns(params);
    ...
    async fetchOneRun(id) {
        ...
        const result = await this.runsService.fetchOneRun(id);
    ...
    async fetchFlpsOfRun(id) {
        ...
        const result = await this.runsService.fetchFlpsOfRun();
    ...
    /**
     * Setter for singular run data.
     * @param {Object} run The new run value to set the internal run to.
     */
    set run(run) {
        this._run = run;
    }
