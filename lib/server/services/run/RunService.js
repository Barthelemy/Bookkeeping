...
const { getAllDetectorsByNameOrFail } = require('../detector/getAllDetectorsByNameOrFail');
...
class RunService {
     ...
     * @param {string|null} [relations.runTypeName] if not null, the name of the created run's type
    ...
    async create(newRun, relations) {
        ...
        const detectors = newRun.detectors
            ? await getAllDetectorsByNameOrFail(newRun.detectors.split(',')
    ...
    async update(identifier, runPatch, relations) {
        const { tagsTexts = null, eorReasons = null, runTypeName = null, userIdentifier = {} } = relations || {};
        ...
        return this.get(identifier, { tags: true, eorReasons: true });
    }
}
