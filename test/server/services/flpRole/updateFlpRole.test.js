...
const { updateFlpRole } = require('../../../../lib/server/services/flp/updateFlpRole.js');
...
module.exports = () => {
    it('should successfully update a given FLP role', async () => {
        ...
        {
            const updatedFlpRole =  await updateFlpRole({ flpRoleId }, {
                nTimeframes: 100,
                bytesProcessed: 2048,
        ...
        {
            const updatedFlpRole = await updateFlpRole({ flpName, runNumber }, {
                nTimeframes: 50,
                bytesProcessed: 1024,
